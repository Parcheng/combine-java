...
public interface ToolMonitorLogicConfig extends ILogicConfig {

    @Field(key = "keyConfig", name = "各监控指标KEY配置", type = FieldTypeEnum.CONFIG, isRequired = true)
    @FieldObject(KeyConfig.class)
    KeyConfig keyConfig();

    @Field(key = "components", name = "监听后要执行的组件集合（ID或组件配置）", type = FieldTypeEnum.COMPONENT, isArray = true, isRequired = true)
    String[] components();
