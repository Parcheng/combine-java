...
@Component(key = "packages", name = "包装组件", logicConfigClass = LogicPackagesLogicConfig.class, initConfigClass = IInvalidInitConfig.class)
@ComponentDesc("用于将多个组件包装成一个组件来执行，并可以指定执行结果")
@ComponentResult(name = "配置的输出结果")
public class LogicPackagesComponent extends AbstractComponent<IInvalidInitConfig, LogicPackagesLogicConfig> {

    public LogicPackagesComponent() {
        super(IInvalidInitConfig.class, LogicPackagesLogicConfig.class);
    }
