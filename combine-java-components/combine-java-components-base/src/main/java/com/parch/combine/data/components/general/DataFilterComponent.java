 ...
 * 数据过滤组件
 */
@Component(key = "filter", name = "数据过滤组件", logicConfigClass = DataFilterLogicConfig.class, initConfigClass = IInvalidInitConfig.class)
@ComponentResult(name = "参数 resultId 指定组件的结果，或上一步组件的结果")
public class DataFilterComponent extends AbstractComponent<IInvalidInitConfig, DataFilterLogicConfig> {

    /**
     * 构造器
     */
    public DataFilterComponent() {
        super(IInvalidInitConfig.class, DataFilterLogicConfig.class);
    }
