...
public abstract class AbsRedisComponent<T extends RedisInitConfig, R extends LogicConfig> extends AbsComponent<T, R> {
    ...
    public List<String> init() {
        ...
        // 初始化
        if (initConfig.getTimeout() == null) {
            initConfig.setTimeout(2000);
        }
        if (initConfig.getMaxAttempts() == null) {
            initConfig.setMaxAttempts(10);
        }
        if (initConfig.getPool() != null) {
            if (initConfig.getPool().getMax() == null) {
                initConfig.getPool().setMax(10);
            }
            if (initConfig.getPool().getMin() == null) {
                initConfig.getPool().setMin(5);
            }
            if (initConfig.getPool().getTimeout() == null) {
                initConfig.getPool().setTimeout(10000L);
            }
        }
