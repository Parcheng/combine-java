...
public class LogicLoopComponent extends AbsComponent<LogicLoopInitConfig, LogicLoopLogicConfig> {
    ...
    public List<String> init() {
        ...
        if (logicConfig.getFailStop() == null) {
            logicConfig.setFailStop(true);
        }
        ...
        } else if (logicConfig.getCount() < 0) {
    ...
    public DataResult execute() {
        ...
        for (int i = 0; i < logicConfig.getCount(); i++) {
            ...
            if (logicConfig.getCondition() != null) {
                ...
                // 判断是否需要跳过当前循环
                if (CompareHelper.isPass(loopCondition.getSkip())) {
                    continue;
                }

                // 判断是否停止循环
                if (CompareHelper.isPass(loopCondition.getFinish())) {
                    break;
                }
            }
