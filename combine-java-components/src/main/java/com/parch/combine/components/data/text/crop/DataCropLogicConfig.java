...
public class DataCropLogicConfig extends LogicConfig {
    ...
    @ComponentField(key = "startRow", name = "从第几行开始读取", type = FieldTypeEnum.NUMBER, defaultValue = "1")
    @ComponentFieldEg(eg = "2", desc = "从第二行开始读取，可以用于跳过表头")
    private Integer startRow;

    @ComponentField(key = "startIndex", name = "从行的第几项开始读取", type = FieldTypeEnum.NUMBER, defaultValue = "1")
    @ComponentFieldEg(eg = "1", desc = "从首行第一项开始读取")
    private Integer startIndex;

    @ComponentField(key = "startSkipCount", name = "每行跳过前几项", type = FieldTypeEnum.NUMBER, defaultValue = "0")
    @ComponentFieldDesc("提示：表格文件表示跳过前几列，文本文件表示跳过前几个字符")
    @ComponentFieldEg(eg = "5", desc = "读取时每行跳过前五项")
    private Integer startSkipCount;

    @ComponentField(key = "endDiscardCount", name = "每行丢弃掉后几项", type = FieldTypeEnum.NUMBER, defaultValue = "0")
    @ComponentFieldEg(eg = "5", desc = "读取时每行舍弃掉最后五项")
