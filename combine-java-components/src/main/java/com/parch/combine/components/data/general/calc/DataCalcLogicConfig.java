...
public class DataCalcLogicConfig extends LogicConfig {
    ...
    @FieldDesc("计算配置项集合")
    @FieldGroup(index = 0, name = "计算结果要写入的数据源（“-”表示不写入）", type = FieldTypeEnum.TEXT, hasExpression = true)
    @FieldGroup(index = 1, name = "要使用的计算函数", type = FieldTypeEnum.SELECT)
    @FieldGroupSelect(index = 1, enumClass = DataCalcModeEnum.class)
    @FieldGroup(index = 2, name = "函数参数（多个用空格分隔）", type = FieldTypeEnum.TEXT, hasExpression = true)
