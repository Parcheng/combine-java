...
public class DataEditLogicConfig extends LogicConfig {
    ...
    @FieldDesc("编辑配置项集合")
    @FieldGroup(index = 0, name = "要编辑的数据源", type = FieldTypeEnum.TEXT, hasExpression = true)
    @FieldGroup(index = 1, name = "数据编辑方式", type = FieldTypeEnum.SELECT)
    @FieldGroupSelect(index = 1, enumClass = DataEditTypeEnum.class)
    @FieldGroup(index = 2, name = "参数，多个使用空格分隔", type = FieldTypeEnum.TEXT, hasExpression = true)
