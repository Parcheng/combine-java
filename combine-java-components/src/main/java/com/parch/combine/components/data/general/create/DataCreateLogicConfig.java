...
public class DataCreateLogicConfig extends LogicConfig {
    ...
    @FieldDesc("创建配置项集合")
    @FieldGroup(index = 0, name = "创建数据的被赋值变量", type = FieldTypeEnum.TEXT, hasExpression = true)
    @FieldGroup(index = 1, name = "数据类型", type = FieldTypeEnum.SELECT)
    @FieldGroupSelect(index = 1, enumClass = DataTypeEnum.class)
    @FieldGroup(index = 2, name = "数据值。如果是集合或对象类型值可以有多个（空格分隔），对象类型值必须为“字段名:字段值”格式", type = FieldTypeEnum.TEXT, hasExpression = true)
